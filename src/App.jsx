...
import { Shield, Terminal, Lock, AlertTriangle, FileText, Code, CheckCircle, Circle, Menu, X, ChevronRight, Key, Network } from 'lucide-react';
import LinuxFundamentals from './pages/LinuxFundamentals';
import NetworkingFundamentals from './pages/NetworkingFundamentals';

// ============= Home Page =============
const HomePage = ({ completedLessons, courses }) => {
...
// ============= OWASP Page =============
const OwaspPage = ({ owaspLessons, completedLessons }) => {
  const navigate = useNavigate();
      ...
      <div className="grid gap-4">
        {owaspLessons.map((lesson, index) => {
          ...
          return (
            <div 
              key={lesson.id}
              onClick={() => isActive && navigate(`/owasp/${lesson.id}`)}
...
const InfoDisclosurePage = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4 mb-6">
        <Link to="/owasp" className="p-2 hover:bg-gray-100 rounded-lg">
          <ChevronRight className="w-6 h-6 rotate-180" />
        </Link>
        <div className="flex-1">
...
const App = () => {
  ...
  const courses = [
    ...
    lessons: 2,
    description: 'فهم الشبكات والإنترنت ونموذج OSI'
  },
    {
      id: 'linux',
  ...
  const owaspLessons = [
    ...
    { id: 'broken-auth', title: 'Broken Authentication', subtitle: 'ثغرات المصادقة', icon: Key, status: 'soon' },
    { id: 'injection', title: 'Injection', subtitle: 'ثغرات الحقن', icon: Code, status: 'soon' },
    { id: 'xss', title: 'Cross-Site Scripting', subtitle: 'XSS', icon: AlertTriangle, status: 'soon' }
  ];
  ...
  <Link
    to="/owasp"
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <Routes>
            <Route path="/" element={<HomePage completedLessons={completedLessons} courses={courses} />} />
            <Route path="/linux" element={<LinuxFundamentals />} />
             <Route path="/networking" element={<NetworkingFundamentals />} />
            <Route path="/owasp" element={<OwaspPage owaspLessons={owaspLessons} completedLessons={completedLessons} />} />
            <Route path="/owasp/info-disclosure" element={<InfoDisclosurePage />} />
            <Route path="/owasp/broken-access" element={<BrokenAccessPage />} />
          </Routes>
        </main>
